<template>
  <div class="wrapper">
    ...
    />
    <select v-else :value="currentValue" class="constrained-width" @change="onUpdate($event.target!.value)">
...
const onUpdate = (value: string | number) => {
  ...
  for (const track in soundEngine.tracks) {
    ...
    editedEffect.effect.set(editedEffect.options as unknown as Record<string, number>)
...
onMounted(() => {
  ...
  EFFECTS_OPTIONS[props.effectName].find(_ => _.name === props.fieldName)!.enum
      ?.forEach(_ => selectOptions.value.push(_))
